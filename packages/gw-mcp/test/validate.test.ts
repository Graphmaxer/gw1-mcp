...
const cases: Array<{ rule: string; kind: "errors" | "warnings"; template: object; options?: object }> = [
  { rule: "UNKNOWN_ATTRIBUTE", kind: "errors",
    template: { ...base, attributes: [{ attributeId: 99, rank: 5 }] } },
  { rule: "DUPLICATE_ATTRIBUTE", kind: "errors",
    template: { ...base, attributes: [{ attributeId: 44, rank: 5 }, { attributeId: 44, rank: 6 }] } },
  { rule: "RANK_OUT_OF_RANGE", kind: "errors",
    template: { ...base, attributes: [{ attributeId: 44, rank: 13 }] } },
  { rule: "ATTRIBUTE_PROFESSION_MISMATCH", kind: "errors",
    // Strength (17, Warrior) on a D/any bar.
    template: { ...base, attributes: [{ attributeId: 17, rank: 5 }] } },
  { rule: "PRIMARY_ATTRIBUTE_ON_SECONDARY", kind: "errors",
    // D/W bar allocating Strength (17, Warrior's primary attribute).
    template: { ...base, secondary: 1, attributes: [{ attributeId: 17, rank: 5 }] } },
  { rule: "PVE_ONLY_ON_HERO", kind: "warnings",
  ...
  { rule: "UNALLOCATED_ATTRIBUTE", kind: "warnings",
    // Staggering Force (1519? use Wearying Strike scythe) — Twin Moon Sweep 1489 scales with Scythe (41), unallocated.
    template: { ...base, attributes: [{ attributeId: 44, rank: 12 }], skills: [1518, 1489, 0, 0, 0, 0, 0, 0] } },
];
...
describe("structural validator rules", () => {
  it("raises SAME_PROFESSIONS", () => {
    const r = validateBuild({ primary: 10, secondary: 10, attributes: [], skills: [1518,0,0,0,0,0,0,0] }, {});
    expect(r.errors.map(e=>e.code)).toContain("SAME_PROFESSIONS");
  });
  it("raises INVALID_SKILL_COUNT", () => {
    const r = validateBuild({ primary: 10, secondary: 0, attributes: [], skills: [1518,0,0] }, {});
    expect(r.errors.map(e=>e.code)).toContain("INVALID_SKILL_COUNT");
  });
  it("raises UNKNOWN_SKILL", () => {
    const r = validateBuild({ primary: 10, secondary: 0, attributes: [], skills: [64321,0,0,0,0,0,0,0] }, {});
    expect(r.errors.map(e=>e.code)).toContain("UNKNOWN_SKILL");
  });
  it("raises DUPLICATE_SKILL", () => {
    const r = validateBuild({ primary: 10, secondary: 0, attributes: [], skills: [1518,1489,1489,0,0,0,0,0] }, {});
