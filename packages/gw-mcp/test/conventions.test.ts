...
describe("tools/list size is a locked, deliberate number (debt #10)", () => {
  ...
  // ({"type":["string","null"]}) instead of 4.4's anyOf pair. That third move is the
  // lock catching a DEPENDENCY, which is what it is for: accepted only after proving
  // the two forms normalise to the same schema and all 8 tools keep their
  // outputSchema, strict input and description.
  const TOOLS_LIST_CHARS = 19_095;
